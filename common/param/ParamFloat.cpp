...
void ParamFloat::setValue(int const val)
{
   *(float*)m_p_value = (float)val;
   LOG(WARNING) << "cast unexpected integer to float";
}
...
void ParamFloat::printInfo(void) const{
   LOG(INFO) << *(float*)m_p_value << " (" << m_descriptorName << "\\" << directionString() << ")";
}
bool ParamFloat::testBounces(void) const{
   return (*(float*)m_p_value <= m_maxValue && *(float*)m_p_value >= m_minValue);
}
int ParamFloat::getBounces(float& min,float& max) const{
   min = m_minValue;
   max = m_maxValue;
   return(0);
}
